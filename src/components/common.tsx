      ...
      <div className="container-lux grid grid-cols-2 gap-y-4 py-6 md:grid-cols-4">
        {items.map((label, i) => (
          <div key={label} className="flex items-center gap-3 px-2">
            ...
            <span className={`text-[11px] uppercase tracking-[0.14em] ${light ? 'text-charcoal/70' : 'text-sand/60'}`}>{label}</span>
          </div>
        ))}
      </div>
    </div>
...
export function DisclaimerNote({ light = false, className = '' }: { light?: boolean; className?: string }) {
  return (
    <p className={`text-xs leading-relaxed ${light ? 'text-charcoal/70' : 'text-sand/60'} ${className}`}>
